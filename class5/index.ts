...
let num: number[] = [2, 5, 9, 44, 72, 75];

const insertValueAtIndex = (array: number[], index: number, value: number): number[] => {
  array.splice(index, 0, value);
  return array;
};
let modifiedResult: number[] = insertValueAtIndex(num, 4, 65);
...
let cart: string[] = ["Pasta", "Burger", "Rice", "Soups", "Cheese", "Egg"];

const addItem = (cart: string[],index:number,deleteRow:number,value:string) => {
    cart.splice(index,deleteRow,value); // Add item
    console.log(cart);
}
const removeItem = (cart: string[],index:number,deleteRow:number,) => {
    cart.splice(index,deleteRow); // Remove item
    console.log(cart);
}
const updateItem = (cart: string[],index:number,deleteRow:number, value:string) => {
...
let negArray: number[] = [1, 4, 7, 9, 0, 4, 7, -3, 7, -8];

const negativeCheck = (value: number[]): number[] => {
...
let celsiusTemperatures: number[] = [20, 54, 10, 7, 39];

const convertToCelsius = (celsiusTemps: number[]): number[] => {
